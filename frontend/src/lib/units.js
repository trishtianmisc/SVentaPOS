...
const MEASUREMENT_SET = new Set(MEASUREMENT_UNITS);
 ...
 */
export function classifyUnit(unitName) {
    const n = (unitName ?? '').trim().toLowerCase();
    if (!n)
        return 'count';
    if (MEASUREMENT_SET.has(n))
        return 'measurement';
    if (COUNT_SET.has(n))
        return 'count';
    return 'count';
}
export function isMeasurementUnit(unitName) {
    return classifyUnit(unitName) === 'measurement';
}
/** +/- step for a unit: 0.01 for kg/L-style, 1 for packs/cases. */
export function qtyStep(unitName) {
 ...
 */
export function roundQty(qty, unitName) {
 ...
 */
export function unitFactor(unitName, units, baseUnit = DEFAULT_BASE_UNIT) {
    if (!unitName)
        return 1;
    if (unitName.trim().toLowerCase() === baseUnit.trim().toLowerCase())
        return 1;
    const match = units?.find((u) => u.unit_name.toLowerCase() === unitName.toLowerCase());
    if (!match)
        return 1;
 ...
 *   next = base / toFactor
 */
export function convertQtyBetween(qty, fromUnit, toUnit, units, baseUnit = DEFAULT_BASE_UNIT) {
...
export function baseEquivalent(qty, unitName, units, baseUnit = DEFAULT_BASE_UNIT) {
 ...
 */
export function baseUnitName(product) {
...
export function validateUnitDraft(row, existingNames, selfId) {
    ...
    const dup = existingNames.some((n) => n.toLowerCase() === lower && n !== (selfId ? undefined : undefined));
    // Duplicate check against other rows (caller passes other names).
    if (dup)
        return `Duplicate unit name "${name}".`;
    return null;
}
/** Names of all other rows except this draft (for duplicate detection). */
export function otherUnitNames(rows, excludeId, idFactory) {
    return rows
