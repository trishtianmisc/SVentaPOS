...
export const FEATURE_PERM_COUNT = {
    dashboard: 2,
    pos: 4,
    inventory: 4,
    customers: 3,
...
export function mergeMatrix(raw) {
...
export function apiRoleFor(matrixRole) {
    return matrixRole === 'staff' ? 'inventory' : matrixRole;
}
/** Map API role → matrix column (inventory → staff); owner has no column. */
export function matrixRoleFor(apiRole) {
    if (apiRole === 'inventory')
        return 'staff';
    if (apiRole === 'manager' || apiRole === 'cashier')
        return apiRole;
    return null;
}
 ...
 */
export function takeLegacyMatrix(orgId) {
    try {
