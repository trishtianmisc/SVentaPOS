-- VentaPOS Phase 6 follow-up: fix the idempotent-replay branch of
-- complete_sale. Since 0004 it selected the replay jsonb payload INTO the
-- uuid variable v_sale_id, so ANY retried sale (same idempotency key, e.g.
-- after a network timeout) raised
-- "invalid input syntax for type uuid" instead of returning the prior sale.
-- The replay payload now goes into a dedicated jsonb variable. Signature
-- unchanged, so existing grants (see 0013) are preserved.
-- (Body is otherwise identical to 0016; column/constraint DDL is
-- idempotent via IF NOT EXISTS / duplicate_object guards.)
