-- VentaPOS Phase 2: complete_sale v2
-- Adds: wholesale tier pricing, utang (credit) payments with ledger posting,
-- credit-limit check with audited manager override. Atomic, idempotent.

-- Drop v1 overload so only the new signature exists.
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    v_line := round(v_price * v_qty - least(v_disc, v_price * v_qty), 2);
    v_subtotal := v_subtotal + round(v_price * v_qty, 2);
    v_item_disc := v_item_disc + least(v_disc, v_price * v_qty);
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    values (v_sale_id, (v_item->>'product_id')::uuid, v_prod.name, 'pc',
      v_qty, v_price, coalesce(v_prod.cost_price, 0),
      least(v_disc, v_price * v_qty), v_line);
