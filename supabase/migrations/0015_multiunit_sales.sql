-- VentaPOS Phase 4: multi-unit selling (C1).
-- sale_items gains unit_quantity (line qty in the sold unit; null = base
-- unit). complete_sale resolves an optional per-line unit_name against
-- product_units: stock math and the wholesale tier always run on BASE units,
-- while the line price uses the unit's selling_price (else linear fallback
-- from the base price). Signature unchanged, so existing grants (see 0013)
-- are preserved by CREATE OR REPLACE.

alter table public.sale_items
  add column if not exists unit_quantity numeric(12,2);

-- Drop v1 overload so only the current signature exists (no-op if gone).
drop function if exists public.complete_sale(uuid,uuid,uuid,jsonb,jsonb,text,uuid,numeric);
...
begin
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    -- Tier always triggers on BASE-unit quantity.
    v_price := v_prod.retail_price;
    if v_prod.wholesale_price is not null
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    insert into public.sale_items(
      ...
      quantity, unit_quantity, unit_price, unit_cost, discount_amount, line_total)
  ...
  return jsonb_build_object(
    ...
    'tax_amount', 0, 'total', v_total, 'paid', v_paid + v_utang,
