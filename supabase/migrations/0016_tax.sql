-- VentaPOS Phase 6: PH VAT (VAT-inclusive only).
-- stores gains tax_status ('non_vat' default | 'vat'), vat_rate (default 12),
-- and an optional tin for the receipt header. products gains vat_exempt.
-- sales snapshots tax_rate + vatable_amount (vat-inclusive net of vatable
-- lines after discounts); sale_items snapshots vat_exempt per line.
-- complete_sale computes: non_vat -> tax 0 (today's behavior); vat ->
-- tax = vatable * rate/(100+rate), carved OUT of the total (inclusive:
-- shelf prices never change at checkout). sale_discount is allocated
-- pro-rata to the vatable bucket. Signature unchanged, so existing grants
